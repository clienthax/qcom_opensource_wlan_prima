...
ifeq ($(KERNEL_BUILD),1)
	...
	MODNAME := wlan
	WLAN_ROOT := vendor/qcom/opensource/wlan/prima

	# keep the feature the same with module build
...
HDD_OBJS +=	$(HDD_SRC_DIR)/hw_wifi.o 
endif
...
CDEFINES :=	-DANI_BUS_TYPE_PLATFORM=1 \
		...
		-DWLAN_FEATURE_PACKET_FILTERING_MDNS \
		-DWLANTL_DEBUG\
		-DWLAN_NS_OFFLOAD \
